...
<title>Bài Tập Lớp</title>
<link rel="stylesheet" href="../css/BaiTapLop.css">
...
<?php
    ...
    if (isset($_POST['submit']))
	{
		...
		&& $FileType != "mp3" && $FileType != "zip" && $FileType != "rar") {
			...
			$STTBai = $_POST['STTBai'];
			move_uploaded_file($_FILES['fileitem']['tmp_name'], "./BaiNop/".$_FILES['fileitem']['name']);
			   insertBaiNop($STTBai, $_SESSION['MaLop'], $user_login, $_FILES['fileitem']['name']);
			//echo "Nộp Bài Thành Công";
		}
	}
	
	$BaiNop = get_BaiNop_mahv($user_login);
...
<div id="Web_1366__1">
	...
	<svg class="Rectangle_15">
		<rect id="Rectangle_15" rx="0" ry="0" x="0" y="0" width="1291" height="1900">
		</rect>
	</svg>
	<div id="Group_18">
	...
	<div id="_Change_Personal_Information">
		<span><br/><a href="#" style="text-decoration: none; color:red">Bài Tập Lớp</a></span>
	</div>
    ...
    <script>
		var baitaplop = <?php echo json_encode($BaiTapLop); ?>;
		var bainop = <?php echo json_encode($BaiNop); ?>;
        var content = "";
        for (i = 0; i <baitaplop.length; i++) {
            var index = i+1;
            content += "<span class=\"bai\">Bài</span> " + index + ":</br>";
            content += "<span class=\"yc\">Yêu cầu:</span> <span class=\"content\">" + baitaplop[i].YeuCau + "</span></br>";
            content += "<span class=\"yc\">Mô tả:</span> <span class=\"content\">" + baitaplop[i].MoTa + "</span></br>";
            if( baitaplop[i].Filename == "" || baitaplop[i].Filename == null){
            }
            else {
                content += "<span class=\"yc\">File:</span> <span class=\"content\"><a href=\'download/dowload_BaiTap.php?filename=" + baitaplop[i].Filename + "\'>" + baitaplop[i].Filename + "</a></span></br>";
            }
			content += "<span class=\"yc\">Nộp bài: </span>";
			var index = 0;
			var getFilename;
			for(var j = 0; j <bainop.length; j++){
				if (bainop[j].STTBai == baitaplop[i].STT){
					index = bainop[j].Filename;
				}
			}
			if (index == 0){
				content += "<form method=\"post\"  enctype=\"multipart/form-data\">";
				content += "<input id=\"file\" type=\"file\" name=\"fileitem\" value=\"Upload File\"\>";
				content += "<input id=\"NopBai\" type=\"submit\" name=\"submit\" value=\"Nộp Bài\">";
				content += "<input type=\"hidden\" name=\"STTBai\" value=\"" + baitaplop[i].STT + "\">";
				content += " </form>";  
			}
			else {
				content += "<a href=\"./download/download_BaiNop.php?filename=" + index + "\" style=\"font-size: 15px;\">" + index + "</a>";
				content += "<form method=\"post\"  enctype=\"multipart/form-data\">";
				content += "<input id=\"NopBai\" type=\"submit\" name=\"delete\" value=\"Xóa\">";
				content += "<input type=\"hidden\" name=\"STTBai\" value=\"" + baitaplop[i].STT + "\">";
				content += "<input type=\"hidden\" name=\"user\" value=\"" + <?php echo json_encode($user_login); ?> + "\">";
				content += " </form>";  
			}	
            content += "</br><hr id=\"TagHR\"></br>";
        }
		//document.getElementById("Bi").innerHTML=content;
